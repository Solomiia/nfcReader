 ...
 */
public class ExcelReader implements IExcelTask {

    Context context;
    private String fileName = "file.xls";
    ExcelWriter excelWriter;
    BankReaderTask bankReaderTask;

    public ExcelReader(Context context)
    {
        this.context = context;
        this.excelWriter = new ExcelWriter(context);
        this.bankReaderTask = new BankReaderTask(context, this);
    }

    static Map<String,ArrayList<Atmosphera>> map = new HashMap<String, ArrayList<Atmosphera>>();

    private void executeBankReaderTask(String city)
    {
        if (!excelWriter.existsData(city))
        {
            bankReaderTask.execute("", city,"Атмосфера");
        }
    }
    ...
    @Override
    public void SetResult(ArrayList<Atmosphera> list) {
